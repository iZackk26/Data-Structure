...
public:
  ...
  Vowels(string vowel, int repeat) {
    this->vowel = vowel;
    this->repeat = repeat;
  }

  void setVowel(string vowel) { this->vowel = vowel; }

  void setRepeat() { this->repeat++; }
};

bool searchVowels(Vowels *vowels, string letter) {
  for (int i = 0; i < 5; i++) {
    if (vowels[i].vowel == letter) {
      vowels[i].setRepeat();
      return true;
    }
  }
...
void readPhrase(string phrase, Vowels *vowels) {
  for (int i = 0; i < phrase.length(); i++) {
    string letter = phrase.substr(i, 1);
    if (searchVowels(vowels, letter)) {
...
void printVowels(Vowels *vowels) {
  std::cout << "\n";
  for (int i = 0; i < 5; i++) {
      if (vowels[i].repeat > 0) {
        std::cout << vowels[i].vowel << " = " << vowels[i].repeat << std::endl;
      }
  }
}
...
void game() {
  Vowels *vowels =
      new Vowels[5]{{"a", 0}, {"e", 0}, {"i", 0}, {"o", 0}, {"u", 0}};
  std::string phrase;
  std::cout << "Enter a phrase: ";
  std::getline(std::cin, phrase);

  readPhrase(phrase, vowels);
  printVowels(vowels);
...
void play(Player arr[5]) {
  ...
  for (int i = 0; i < 10; i++) {
    ...
    for (int j = 0; j < 5; j++) {
      ...
      std::cout << arr[j].name << " throwed " << arr[j].cards[maxCard]
                << std::endl;
...
void figure() {
    int filas = 6;
    int caracteres_por_fila = 11;

    for (int i = 0; i < filas; i++) {
        for (int j = 0; j < caracteres_por_fila; j++) {
            if (i % 2 == 0) {
                if (j == 5) {
                    std::cout << "+";
                } else {
